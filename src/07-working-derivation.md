...
> We did it! The contents of `/nix/store/w024zci0x1hh1wj6gjq0jagkc1sgrf5r-foo` is really foo.
...
> Similarly, `inherit gcc coreutils;` is equivalent to ` gcc = gcc; coreutils = coreutils;`. Lastly, `inherit (pkgs) gcc coreutils;` is equivalent to ` gcc = pkgs.gcc; coreutils = pkgs.coreutils;`.
