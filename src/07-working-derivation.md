...
Команда `declare -xp` выводит список экспортированных переменных (`declare` — встроенная функция `bash`).
...
Это список на самом деле содержит один скрипт `builder.sh`, который каким-то образом оказался в хранилище Nix.
...
Поскольку `builder.sh` — обычный файл, с ним не могут быть ассоциированы никакие файлы `.drv`.
Путь в хранилище вычисляется на основании имени файла и хеша его содержимого.
...
Не беспокойтесь о том, откуда возьмутся эти переменные; давайте пока опишем деривацию и соберём её:
...
`nix-build` выполняет две задачи:
...
Во второй строке `simple.nix` у нас есть вызов функции `import`.
...
Подобный вызов мы видели в [пятой пилюле](05-functions-and-imports.md).
...
Тот же эффект возникает и при выполнении инструкции `:l <nixpkgs>`, который мы использовали в `nix repl`.
...
[^1]: *Шебанг* ровно так и записан в русской википедии.
      *Shebang* или *hash-bang*, как в оригинале у Люка — символы "#!", которые идут в начале любого скрипта и указывают пусть к интерпретатору.
