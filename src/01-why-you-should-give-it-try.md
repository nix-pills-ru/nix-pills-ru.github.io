...
Скажем, вам потребуются средства оркестрации, настройка общего кэша пакетов и новые машины для мониторинга всего этого счастья — вместо пары простых сервисов.
...
Деривации/пакеты находятся в хранилище Nix в подкаталогах, чьи имена соответствуют формату `/nix/store/hash-name`, где хеш (hash) — уникальный идентификатор деривации (с некоторыми оговорками, на которые мы не будет отвлекаться), а имя (name) — её имя.
...
Можно создать две деривации с одним и тем же именем, но разными хешами: значение имеет только хеш.

Для чего все эти сложности?
Благодаря им, теперь можно запускать `mysql 5.2` с `glibc-2.18` и `mysql 5.5` с `glibc-2.19`.
Можно использовать модуль с `python 2.7`, собранным `gcc 4.6` и тот же самый модуль — с `python 3`, собранным `gcc 4.8`, в одной и той же системе.
...
Если изменился формат данных, то миграция на новый формат — ответственность автора программы.
