...
3. В конец каждого пути из `$buildInputs` дописываем `bin` и всё вместе добавляем к `PATH`.
   Подробности обсудим чуть позже.
...
Вот так всё просто!
Переменная `buildInputs` в конечном итоге будет содержать нужные нам пути, разделённые пробелом.
...
Чтобы разобраться, как работает этот код, вспомним кое-что о [функциях Nix](05-functions-and-imports.md).
Всё выражение Nix из файла `autotools.nix` превращается в функцию.
...
1. Сначала добавляем в область видимость магический набор атрибутов `pkgs`.
...
- Мы помещаем в переменную `pkgs` импорт, который в предыдущих выражениях помещали в оператор "with".
  Это обычная практика, не стоит её опасаться.
...
- Мы создаём деривацию, указывая только атрибуты `name` и `src`.
  Если проекту нужны другие зависимости в `PATH`, их можно добавить в `buildInputs`, но в примере с `hello.nix` нам это было не нужно.

Обратите внимание, что мы не используем никаких других библиотек.
Нам могут потребоваться флаги компилятора C, чтобы искать включаемые файлы других библиотек.
Также, нам могут потребоваться флаги компоновщика, чтобы искать статические библиотечные файлы.
