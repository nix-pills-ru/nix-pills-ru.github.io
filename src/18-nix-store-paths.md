...
Напомню, что у простейшей деривации, которую вы можете сделать, должны быть указаны имя (`name`), сборщик (`builder`) и система (`system`):
...
В случае, если в `.drv` есть входные деривации, которые ссылаются на другие `.drv`, вместо путей `.drv` используются хеше, рассчитанные по тому же алгоритму.
