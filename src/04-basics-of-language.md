...
Для написания выражений, которые конструируют порождения используется [Язык Nix](https://nixos.org/manual/nix/stable/expressions/expression-language.html).
...
Используя Nix в вашей работе, в качестве бонуса вы получаете все те возможности, о которы я рассказывал в прошлых статьях.
...
Напомню, что Nix — это не только набор утилит для работы с порождениями, но и [чистый ленивый функциональный язык](https://nixos.org/manual/nix/stable/expressions/expression-language.html).
Синтаксис `nix repl` немного отличается от синтаксиса Nix, если речь идёт о присваивании переменных (ведь в функциональных языках не бывает присваиваний).
Просто помните об этом отличии, и оно не будет сбивать вас с толку.
На мой взгляд `nix repl` поможет нам быстрее вкатиться в язык, поскольку он оставляет простые вещи простыми.
...
Этих типов хватит, чтобы собрать целую операционную систему.

Nix является сильно типизированным, но не статически типизированным языком.
То есть, вы не можете смешивать строки и целые числа без предварительного преобразования типа.
...
Читая выражения Nix, вы можете посчитать знаки доллара (`$`) многозначными, но с ними надо разобраться.
...
Внутри небора нельзя ссылаться на другие элементы или на сам набор:
...
Можно считать, что это расширенная версия оператора `using` из C++, или `from module import*` из Python.
...
Если, вы всегда можете обратиться к атрибуту через набор:
...
Здесь значение `a` не требуется, поэтоему ошибка деления на ноль не возникает --- выражение просто не вычисляется.
...
[^1]: Оригинальный термин *set* обычно переводится на русский, как *множество*.
      Но в данном случае термин *множество* вводился бы в заблуждение, поскольку *set* содержит не отдельные элементы, а пары ключ-значения. Такую стркутуру обычно называют называют *словарём* или *ассоциативным массивом*.
