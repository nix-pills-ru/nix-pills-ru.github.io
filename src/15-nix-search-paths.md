...
Почему `nix-env` ведёт себя иначе?
...
- `nix-env` позволяет слить несколько репозиториев в один файл `~/.nix-defexpr`, благодаря чему можно получить доступ ко всем деривации на вашей машине.
...
Это позволяет использовать "абстрактные" пути в выражениях Nix и определять "конкретный" путь с помощью `NIX_PATH` или флага **-I**.
