...
**Если вы используете NixOS, то можете пропустить [следующий](#устанавливаем-что-нибудь) шаг**.
...
Ранее мы выяснили, что `~/.nix-profile/etc` указывает на деривацию `nix-2.1.3`.
...
- Утилита [nix-env](https://nixos.org/manual/nix/stable/command-ref/nix-env.html) управляет окружениями, профилями и их поколениями.
...
Куда мы установили `hello` на самом деле? `which hello` говорит нам `~/.nix-profile/bin/hello`, который указывает на хранилище.
...
Теперь `nix-env -q` не показывает `man`.
...
Для запуска `nix-env` нужно указывать операцию с опциями.
...
Поскольку все компоненты окружения указывают на хранилище, нам необходимо научиться работать с ним.
...
Да, это значит, что окружения также находятся в хранилище, и поскольку они содержат симлинки на `hello`, каждое из них зависит от `hello`.
...
Это — основа развёртывания с помощью Nix. Думаю, вы уже догадываетесь, насколько это мощный и удобный инструмент для развёртывания приложений в облаках (подсказка: `nix-copy-closures` и `nix-store --export`).
...
Эта команда скачает новые выражения Nix (описания пакетов), создаст новое поколение профиля каналов и распакует его в `~/.nix-defexpr/channels`.
...
Далее мы выяснили, как работать с хранилищем.
Мы изучили прямые и обратные зависимости путей в нём.
...
Язык Nix используется для описания того, как строить деривации.
