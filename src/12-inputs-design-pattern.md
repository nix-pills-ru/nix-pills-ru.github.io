# Репозитории пакетов и паттерн Входящие
...
С помощью паттерна `Входящие` мы предлагаем другое решение: пусть пользователь меняет параметр `inputs` выражения.
...
Паттерн "`Входящие`" позволяет настраивать выражения с помощью набора аргументов.
