...
Функция `derivation` в качестве первого аргумента принимает набор.
...
## Отступление про файлы .drv

Что такое файл `.drv`?
Это спецификация, как собрать деривацию, без лишнего языкового шума, который появляется при работе с любыми высокоуровневыми языками, в том числе и с Nix.
...
*Замечание*: хеш в выходном пути в текущей версии Nix вычисляется исключительно на основе входных дериваций, без учёта содержимого.
...
Сборщик не наследует никаких переменных из вашей оболочки, иначе сборки бы страдали от [не-детерминизма](https://wiki.debian.org/ReproducibleBuilds).
...
Кое-что интересное об атрибуте `type`.
...
*Очевидное замечание*: каждый раз, когда мы меняем деривацию, вычисляется новый хеш.
...
Благодаря этой удобной возможности, легко выстраивать зависимости между деривациями.
