...
В предыдущей [двенадцатой пилюле](12-inputs-design-pattern.md) мы рассказали о первом из базовых паттеров проектирования репозиториев.
...
Для этого нам потребуется такая функция `callPackage`, чтобы её можно было вызыать подобным образом:
...
- Способ автоматически объеденить набор атрибутов репозитория и значения, полученные из `functionArgs`.
...
У нас получилось почти всё, что мы хотели: мы вызываем фукнции автоматически, передавая им набор аргументов.
...
Код довольно ясный, не смотря на увеличевшееся количество скобок.
...
Эта пилюля демострирует, как Nix может быть универсальным инструментом для сборки и развёртывания программ и как он подходит для создания репозиториев, которые следуют нашим собственным соглашениями.
